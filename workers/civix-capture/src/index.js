// civix-capture Worker — the Send to CiViX docket/filings backend
// (civix-capture.mycivix.workers.dev), used by send-to-civix.html,
// builder.html's Inbox, and digest.js.
//
// Recovered 24 Sep 2026 from the deployed script via the Cloudflare API: the
// original source project wasn't on this machine or in the repo. This is the
// deployed esbuild bundle as-is — everything above "// src/index.js" is the
// vendored postal-mime email parser, everything below is CiViX's own code.
// Deployed with no_bundle (see wrangler.toml), so this file IS what runs.
...
var JUNK_PREFIX = /^(utm_|mc_|pk_|ref_)/i;
var JUNK_EXACT = ["fbclid", "gclid", "igshid", "mkt_tok", "ref", "source", "si", "_hsenc", "_hsmi", "yclid", "msclkid"];
var TOKEN_RE = /^[a-z0-9]{10}$/;
var TOKEN_ALPHABET = "abcdefghijkmnpqrstuvwxyz23456789";
function newToken() {
...
function canonical(raw) {
  try {
    ...
    u.searchParams.forEach((_, k) => {
      if (JUNK_PREFIX.test(k) || JUNK_EXACT.includes(k.toLowerCase())) drop.push(k);
    });
...
async function hmac(secret, value) {
  ...
  const sig = await crypto.subtle.sign("HMAC", key, new TextEncoder().encode(value.toLowerCase().trim()));
  return [...new Uint8Array(sig)].map((b) => b.toString(16).padStart(2, "0")).join("");
}
__name(hmac, "hmac");
var SIGNOFF_RE = /^(sent from my |sent via |get outlook for |—+\s*$)/i;
...
async function fileItem(env, token, { title, text, url, source }) {
  title = (title || "").trim();
  text = (text || "").trim();
  url = (url || "").trim();
  if (!url) {
  ...
  if (!title && !text && !url) return { status: "empty" };
  const ckey = url ? canonical(url) : "note:" + (title + " " + text).trim().toLowerCase();
  let display, note;
  if (title) {
    display = title;
    note = text && text !== title ? text : "";
  } else if (text) {
    display = text.length > 120 ? text.slice(0, 120).trim() + "\u2026" : text;
    note = text.length > 120 ? text : "";
  } else {
    display = hostOf(url) || url;
    note = "";
  }
  const now = Date.now();
  const existing = await env.DB.prepare("SELECT id, repeats, note FROM filings WHERE token = ? AND ckey = ?").bind(token, ckey).first();
  if (existing) {
    await env.DB.prepare(
      "UPDATE filings SET repeats = repeats + 1, at = ?, note = CASE WHEN note = '' THEN ? ELSE note END WHERE id = ?"
    ).bind(now, note, existing.id).run();
    return { status: "repeat", repeats: existing.repeats + 1 };
  }
  const seqRow = await env.DB.prepare("UPDATE dockets SET seq = seq + 1, last_seen_at = ? WHERE token = ? RETURNING seq").bind(now, token).first();
  const seq = seqRow ? seqRow.seq : 1;
  const id = "f" + now.toString(36) + Math.random().toString(36).slice(2, 7);
  await env.DB.prepare(
    `INSERT INTO filings (id, token, seq, ckey, title, url, host, note, at, repeats, state, source)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, 'docket', ?)`
  ).bind(id, token, seq, ckey, display, url, hostOf(url), note, now, source || "web").run();
...
var index_default = {
  ...
  async email(message, env, ctx) {
    ...
    if (!TOKEN_RE.test(token)) {
      message.setReject("No such CiViX docket");
      return;
    }
    const docket = await env.DB.prepare("SELECT token, sender_hash FROM dockets WHERE token = ?").bind(token).first();
    if (!docket) {
      message.setReject("No such CiViX docket");
      return;
    }
    const fromHash = await hmac(env.SENDER_SECRET, String(message.from || ""));
    if (!docket.sender_hash) {
      await env.DB.prepare("UPDATE dockets SET sender_hash = ? WHERE token = ?").bind(fromHash, token).run();
    } else if (docket.sender_hash !== fromHash) {
      message.setReject("This CiViX address is already in use");
      return;
    }
  ...
  async fetch(request, env, ctx) {
    ...
    // website is https://mycivix.com, the iOS app is capacitor://mycivix.com.
    const allowed = (env.ALLOWED_ORIGIN || "*").split(",").map((o) => o.trim());
    const reqOrigin = request.headers.get("Origin") || "";
    const cors = {
      "Access-Control-Allow-Origin": allowed.includes("*") ? "*" : allowed.includes(reqOrigin) ? reqOrigin : allowed[0],
    ...
    try {
      if (url.pathname === "/api/docket" && request.method === "POST") {
        const token2 = newToken();
        const now = Date.now();
        await env.DB.prepare(
          "INSERT INTO dockets (token, created_at, last_seen_at, seq) VALUES (?, ?, ?, 0)"
        ).bind(token2, now, now).run();
        return json({ token: token2, address: `${token2}@${env.CAPTURE_DOMAIN}` });
      }
      const token = (url.searchParams.get("token") || "").toLowerCase();
      if (!TOKEN_RE.test(token)) return json({ error: "bad token" }, 400);
      const docket = await env.DB.prepare("SELECT token FROM dockets WHERE token = ?").bind(token).first();
      ...
      if (url.pathname === "/api/filing" && request.method === "DELETE") {
        ...
        await env.DB.prepare("DELETE FROM filings WHERE id = ? AND token = ?").bind(id, token).run();
        return json({ ok: true });
      }
      if (url.pathname === "/api/vcard") {
        const address = `${token}@${env.CAPTURE_DOMAIN}`;
        const vcf = [
          "BEGIN:VCARD",
          "VERSION:3.0",
          "N:CiViX;;;;",
          "FN:CiViX",
          "ORG:CiViX",
          `EMAIL;type=INTERNET;type=pref:${address}`,
          "NOTE:Share anything here and it lands on your CiViX docket.",
          "END:VCARD"
        ].join("\r\n");
        return new Response(vcf, {
          headers: {
            ...
            "Content-Disposition": 'attachment; filename="CiViX.vcf"'
          }
        });
      }
