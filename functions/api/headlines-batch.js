...
Reply with ONLY a JSON object, no markdown fences, no other text:
{
  ...
  "talkingPoint": one clear, opinionated position statement (under 20 words) a citizen can swipe agree or disagree with — a single, standalone claim ("Landlords shouldn't be allowed to raise rent more than 5% a year."), not a neutral summary of what's at stake, and NOT a comparison or debate between two sides. Never phrase it as "X shouldn't do Y, even though Z thinks otherwise," "despite critics, X should Y," or anything else that names or nods to an opposing party's view — a citizen reading it shouldn't have to wonder whether swiping right means agreeing with the claim or siding with the group being dismissed. Bad: "Landlords shouldn't be allowed to raise rents that much, even though tenants say it's still not enough." Good: "Landlords shouldn't be allowed to raise rent more than 5% a year." If the headline's own specifics are ambiguous, procedural, or too tangled to turn into one clean, standalone position, zoom out and state a clear position on the broader policy area instead of forcing a confusing one from the details,
